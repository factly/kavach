...
// @Router /organisations/{organisation_id}/applications/{application_id}/default [delete]
func deleteDefault(w http.ResponseWriter, r *http.Request) {
	...
	if !containsApp {
		tx.Rollback()
		loggerx.Error(errors.New("organisation is not part of application"))
		errorx.Render(w, errorx.Parser(errorx.CannotSaveChanges()))
		return
	}

	err = tx.Model(&app).Association("Organisations").Replace(&organisations)
	if err != nil {
		tx.Rollback()
		loggerx.Error(err)
		errorx.Render(w, errorx.Parser(errorx.DBError()))
