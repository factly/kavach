...
func delete(w http.ResponseWriter, r *http.Request) {
	...
	for _, orgUser := range orgUsers {
		...
		// Also hard delete the organisation user record to avoid FK constraints
		err = tx.Unscoped().Delete(&orgUser).Error
		if err != nil {
			tx.Rollback()
			loggerx.Error(err)
			errorx.Render(w, errorx.Parser(errorx.DBError()))
	...
	userToDelete.ID = uint(uID)

	// First get the KID before deleting
	var kid string
	err = tx.Model(&model.User{}).Where("id = ?", uID).Pluck("kid", &kid).Error
	if err != nil {
		tx.Rollback()
		loggerx.Error(err)
		errorx.Render(w, errorx.Parser(errorx.DBError()))
	...
	// Delete user
	err = tx.Unscoped().Delete(userToDelete).Error
	if err != nil {
		tx.Rollback()
		loggerx.Error(err)
		errorx.Render(w, errorx.Parser(errorx.DBError()))
