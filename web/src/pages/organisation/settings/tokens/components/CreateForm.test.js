...
describe("Organisation Token Create Form component", () => {
	...
	describe("functionality", () => {
		it("should call addOrganisationToken on submit", (done) => {
			...
			act(() => {
				component.find("input").at(0).simulate("change", { target: { value: "test token" } });
				component.find("TextArea").at(0).simulate("change", { target: { value: "test token" } });
				component.find("form").simulate("submit");
			});
			setTimeout(() => {
				expect(addOrganisationToken).toHaveBeenCalledWith({ name: "test token", description: "test token" }, setState, setState);
