...
describe("Organisation Settings Roles Create Form component", () => {
	...
	describe("functionality", () => {
		it("should call createOrganisationRole on submit", (done) => {
			...
			act(() => {
				// input name (0)
				component.find('input').at(0).simulate('change', { target: { value: 'test role' } });
				// input slug (1)
				component.find('input').at(1).simulate('change', { target: { value: 'test-role' } });
			...
			setTimeout(() => {
				expect(createOrganisationRole).toHaveBeenCalledWith({ name: 'test role', slug: 'test-role', description: 'test description' })
