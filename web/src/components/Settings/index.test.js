...
describe('SettingsList', () => {
  ...
  it('should render and match snapshot with props passed', () => {
    const wrapper = shallow(
      <Router>
        <SettingsList type="organisation" orgID="1" appID="1" spaceID="1" role="admin" />
      </Router>,
    );
    expect(wrapper).toMatchSnapshot();
  });
  ...
  it('should make link to application settings', () => {
    const wrapper = render(
      <Router>
        <SettingsList type="application" orgID="1" appID="1" spaceID="1" role="admin" />
      </Router>,
    );
    const links = wrapper.find('a');
    Array.from(links).forEach((link) => {
  ...
  it('should make link to space settings', () => {
    const wrapper = render(
      <Router>
        <SettingsList type="space" orgID="1" appID="1" spaceID="1" role="admin" />
      </Router>,
    );
    const links = wrapper.find('a');
    Array.from(links).forEach((link) => {
  ...
  it('should render links for owner', () => {
    const wrapper = render(
      <Router>
        <SettingsList type="space" orgID="1" appID="1" spaceID="1" role="owner" />
      </Router>,
    );
    const links = Array.from(wrapper.find('a'));
    expect(links[links.length - 1].attribs.href).toEqual(
      '/applications/1/settings/spaces/1/settings/tokens',
    );
  });
  ...
  it('should make not baseLink for other types', () => {
    const wrapper = render(
      <Router>
        <SettingsList type="other" orgID="1" appID="1" spaceID="1" role="admin" />
      </Router>,
    );
    const links = wrapper.find('a');
